...
class BudgetRepository {
  ...
  Stream<Map<String, int>> watchBudgets() => _db
      .select(_db.budgets)
      .watch()
      .map((rows) => {for (final r in rows) r.bulanKey: r.nominal});

  Stream<Map<String, int>> watchTargets() => _db
      .select(_db.savingsGoals)
      .watch()
      .map((rows) => {for (final r in rows) r.bulanKey: r.targetNominal});
  ...
  Future<void> hapusTarget(String bulanKey) => (_db.delete(
    _db.savingsGoals,
  )..where((s) => s.bulanKey.equals(bulanKey))).go();
}
